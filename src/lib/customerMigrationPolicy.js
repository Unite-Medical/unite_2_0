...
export function buildMigratedCustomer(source, { certificate_status = 'unknown', contract_prices = null, addresses = null, agreement = null } = {}) {
  ...
  const agreementVerified = agreement?.source_verified === true && ['shopify','sparklayer'].includes(agreement?.source_system) && String(agreement?.source_customer_id) === shopifyId && /^[a-f0-9]{64}$/i.test(agreement?.source_sha256 || '') && agreement?.coverage_complete === true && ['prepay','net15','net30','net60'].includes(agreement?.terms);
  const pricingMissing = contractPrices.length === 0 && !(agreementVerified && agreement?.pricing_basis === 'standard');
  const ready = agreementVerified && !pricingMissing;
  const organization = {
    id: organizationId,
    name: company || name || `Shopify customer ${shopifyId}`,
    source: 'shopify_migration',
    shopify_customer_id: shopifyId,
    approval_status: ready ? 'approved' : 'manual_review',
    ...(ready ? { terms: agreement.terms, tier: agreement.tier || 'C', credit_limit: agreement.credit_limit ?? 0, agreement_source: agreement.source_system, agreement_source_sha256: agreement.source_sha256 } : {}),
    status: 'active',
    auto_joined_by_domain: false,
    pricing_reconciliation_status: pricingMissing ? 'source_missing' : 'preserved',
    commerce_hold_reason: ready ? null : pricingMissing ? 'customer_pricing_source_missing' : 'agreement_source_verification_required',
    shopify_tax_exempt: yes(source['Tax Exempt'] ?? source.taxExempt),
    certificate_status,
  };
