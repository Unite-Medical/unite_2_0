    ...
    return (
      <div style={{ background: D.paper, fontFamily: D.sans, color: D.ink, minHeight: '100vh' }}>
        ...
        <main style={{ maxWidth: 640, margin: '0 auto', padding: '120px 24px', textAlign: 'center' }}>
          ...
          <button onClick={() => navigate('/dashboard')} style={{ marginTop: 16, background: D.plum, color: D.paper, border: 'none', padding: '13px 22px', borderRadius: 4, cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>Back to dashboard</button>
        </main>
      </div>
    );
  }
        ...
        <div style={{ maxWidth: 1360, margin: '0 auto', padding: `24px ${padX}px ${isMobile ? 56 : 64}px`, display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 380px', gap: isMobile ? 22 : 32 }}>
          <div>
            <div style={{ height: isMobile ? 160 : 220, borderRadius: 16, border: `1px solid ${D.line}`, background: D.paperAlt, position: 'relative', overflow: 'hidden', marginBottom: isMobile ? 18 : 24 }}>
              <div style={{ position: 'absolute', inset: 0, background: 'repeating-linear-gradient(45deg, transparent 0 18px, rgba(29,92,77,0.04) 18px 19px)' }} />
              ...
              {PROGRESSION.map((p, i) => {
                ...
                return (
                  <div key={p.status} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 160px', gap: 14, padding: '14px 0', borderTop: i === 0 ? 'none' : `1px solid ${D.line}`, alignItems: 'start' }}>
                    <div>
                      <div style={{ width: 16, height: 16, borderRadius: 8, background: done ? D.plum : D.line, marginTop: 2, boxShadow: now ? '0 0 0 6px rgba(29,92,77,.18)' : 'none' }} />
                    </div>
                    <div>
          ...
          <div style={{ display: 'grid', gap: 14, alignContent: 'start' }}>
            ...
            <button onClick={() => navigate('/contact')} style={{ background: D.plum, color: D.paper, border: 'none', padding: 14, borderRadius: 4, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Contact rep about this order</button>
            <button style={{ background: 'transparent', color: D.ink, border: `1.5px solid ${D.ink}`, padding: 13, borderRadius: 4, fontSize: 14, cursor: 'pointer' }}>Download packing slip</button>
          </div>
        </div>
      </main>
      <Footer />
