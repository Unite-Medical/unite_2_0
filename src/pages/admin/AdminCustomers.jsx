            ...
            <>
              ...
              <div style={{ marginTop: 18, padding: 16, background: D.paper, borderRadius: 10, border: `1px solid ${D.line}`, display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 12 }}>
                ...
                <EditField label="Account rep">
                  <select aria-label="Account rep" value={active.account_rep || ''} onChange={(e) => patchOrg({ account_rep: e.target.value })} style={inputStyle}>
                    <option value="">Unassigned</option>{repNames.map((r) => <option key={r} value={r}>{r}</option>)}
                  </select>
                </EditField>
