...
export function AdminReceiving() {
  useSEO({title:'Receiving · Team workspace',noindex:true});
  ...
  const receivable=!!po&&['sent','partial'].includes(po.status);
  ...
  async function handleScan() {
    ...
    if (!response.ok) {
      if(response.status>=500)throw new Error('Barcode lookup is unavailable. Keep the carton aside and try again.');
      const exceptionKey = `dock_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
      const held=await fetch('/api/wms/dock-exceptions', {
      ...
      if(!held.ok)throw new Error('The barcode could not be resolved and the dock hold could not be saved. Keep the carton aside and contact operations.');
    ...
    } catch(error){setPending(null);setMsg({kind:'err',text:error.message||'Could not check the barcode. No stock was received.'});}
  }

  function addToQueue() {
    if (!pending) { handleScan(); return; }
    const qty = Number(scanQty);
    if (!Number.isInteger(qty)||!(qty > 0)) { setMsg({ kind: 'err', text: 'Enter a positive whole-number quantity.' }); return; }
  ...
  async function post() {
    ...
    try {
      ...
      if (!res.ok) { setMsg({ kind: 'err', text: ({work_unavailable:'Receiving is temporarily unavailable. Your staged quantities are still here; try again when the connection returns.',overage_requires_manager:'Quantity exceeds the remaining purchase order. Remove the staged line and review the delivery with your manager.',purchase_order_changed_retry:'This purchase order changed. Keep the delivery on hold and reload its current quantities.',purchase_order_ap_locked:'Finance is matching this purchase order. Wait for that review to finish before receiving.',receive_unreachable:'Could not connect. Your staged quantities are still here. Check your connection before retrying.'}[res.reason] || `Receive failed: ${res.reason}`) }); return; }
      setMsg({ kind: 'ok', text: res.duplicate ? 'This receipt was already recorded. No stock was added again. Refresh to see the latest quantities.' : `Received ${res.received} unit(s) against ${poId}. PO is now ${res.po_status}. ${res.events} scan(s) logged.` });
      setQueue([]);
  ...
  return (
    <AdminShell active="receiving" unsavedChanges={queue.length>0||!!pending}>
      <main id="main" style={{ padding: `${isMobile ? 24 : 40}px ${padX}px 64px`, maxWidth: 940, margin: '0 auto' }}>
        ...
        <AdminCard title="01 · Against which order?">
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 200px', gap: 12 }}>
            <div>
              ...
              <select aria-label="Open purchase order" disabled={queue.length>0||busy} value={poId} onChange={(e) => { setPoId(e.target.value); setPending(null); setQueue([]); setReceiptKey(newReceiptKey()); }} style={INPUT}>
                <option value="">Select an open purchase order…</option>
                {po&&!receivable&&<option value={po.id} disabled>{po.id} · {po.status} · receiving complete</option>}
                {openPos.map((p) => <option key={p.id} value={p.id}>{p.id} · {p.vendor_name} ({p.status})</option>)}
              </select>
            </div>
            <div>
              <label style={LABEL}>WAREHOUSE</label>
              <select aria-label="Warehouse" disabled={queue.length>0||busy} value={warehouse} onChange={(e) => setWarehouse(e.target.value)} style={INPUT}>
        ...
        {!openPos.length&&<p className="uw-notice">No open purchase orders are available for receiving.</p>}
        <AdminCard title="02 · Scan a carton">
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 120px 120px', gap: 12, alignItems: 'end' }}>
            <div>
              ...
              <input
                aria-label="Scan UPC GTIN or barcode" ref={scanRef}
                autoFocus
                value={scanText}
                onChange={(e) => { setScanText(e.target.value); setPending(null); }}
                onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleScan(); } }}
                placeholder={receivable ? 'Scan or type a UPC / barcode…' : 'Select a purchase order first'}
                disabled={!receivable}
                style={INPUT}
              />
            </div>
            <div>
              ...
              <input type="number" inputMode="numeric" aria-label="Quantity to receive" value={scanQty} onChange={(e) => setScanQty(e.target.value)} placeholder="0" disabled={!receivable} style={INPUT} />
            </div>
            <button onClick={addToQueue} disabled={!receivable} style={{ background: po ? D.ink : D.ink3, color: D.paper, border: 'none', padding: '12px 16px', borderRadius: 4, cursor: po ? 'pointer' : 'not-allowed', fontSize: 14, fontWeight: 600 }}>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 10, marginTop: 10 }}>
                <div>
                  ...
                  <input aria-label="Lot number" value={manualLot} onChange={(e) => setManualLot(e.target.value)} placeholder="Scan/type lot number" style={INPUT} />
                ...
                <div>
                  ...
                  <input aria-label="Expiration date" value={manualExpiration} onChange={(e) => setManualExpiration(e.target.value)} placeholder="YYYY-MM-DD or N/A" style={INPUT} />
          ...
          {pending&&<button className="uw-button quiet" onClick={()=>{setPending(null);setScanText('');setManualLot('');setManualExpiration('');setNotApplicableReason('');setMsg(null);}}>Clear current scan</button>}
          {msg && <div role="status" style={{ marginTop: 12, padding: 10, borderRadius: 8, fontSize: 13, background: msgBg[msg.kind], color: msgFg[msg.kind] }}>{msg.text}</div>}
          ...
          <>
            ...
            <AdminCard title={`03 · Staged (${queue.length})`}>
              ...
              <button onClick={post} disabled={busy || !canReceive} title={canReceive ? '' : 'Insufficient role'} style={{ marginTop: 16, width: '100%', background: canReceive ? D.plum : D.ink3, color: D.paper, border: 'none', padding: 14, borderRadius: 4, cursor: busy ? 'wait' : canReceive ? 'pointer' : 'not-allowed', fontSize: 15, fontWeight: 600 }}>
                {busy ? 'Posting…' : `Confirm received quantities (${queue.reduce((a, r) => a + Number(r.qty), 0)} units)`}
              </button>
            </AdminCard>
          </>
        )}
        ...
        {po && recon && recon.ok && (
          <>
            ...
            <AdminCard title={`04 · Reconcile ${po.id}`}>
              ...
              <div style={{ marginTop: 10, fontSize: 12, color: D.ink3, lineHeight: 1.5 }}>
                Received totals reflect saved receipts. Staged items do not change stock until you confirm receipt. Remove staged items before changing the purchase order or warehouse.
              </div>
            </AdminCard>
          </>
        )}
