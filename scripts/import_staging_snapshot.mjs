...
for(let i=0;i<sourceRows.length;i+=200){const batch=sourceRows.slice(i,i+200);const result=await request({action:'verify_source',run_id:run,rows:batch});if(result.matched!==batch.length)throw new Error('Source payload readback mismatch');matched+=result.matched;}
