...
query Products($cursor: String) {
  products(first: 10, after: $cursor, sortKey: ID) {
    ...
    nodes {
      ...
      media(first: 50) { pageInfo { hasNextPage endCursor } nodes { alt mediaContentType preview { image { url altText width height } } } }
      variants(first: 100) { pageInfo { hasNextPage endCursor }
        nodes {
          id legacyResourceId title sku barcode price compareAtPrice taxable
          inventoryPolicy position selectedOptions { name value }
          image { url altText }
          inventoryItem {
            id tracked requiresShipping countryCodeOfOrigin provinceCodeOfOrigin measurement { weight { value unit } }
            inventoryLevels(first: 10) { pageInfo { hasNextPage endCursor } nodes { id quantities(names: ["available", "committed", "incoming", "on_hand", "reserved"]) { name quantity updatedAt } location { id legacyResourceId name address { address1 address2 city provinceCode zip countryCode } } } }
          }
        }
      }
    }
...
query Orders($cursor: String) {
  orders(first: 50, after: $cursor, query: "status:any", sortKey: ID) {
    pageInfo { hasNextPage endCursor }
    nodes {
