import { canSetNewPrice, isDamon, isJacobe } from '../_lib/launchPolicy.js';
...
export function planContractPriceAction({ organization, product, existing = null, action, unit_price, min_qty = 1, effective_from = null, effective_to = null, actorId, actor = null, now = new Date() } = {}) {
  ...
  const floor = Math.ceil(cost / 0.65 * 100) / 100;
  ...
  if (action === 'set') {
    ...
    const authority = canSetNewPrice(actor, price, cost);
...
export default async function handler(req, res) {
  ...
  try {
    const live = await authorizeLiveRequest(req, sql, { roles: ['admin', 'sales', 'sales_manager'] });
    ...
    const plan = planContractPriceAction({
      ...
      actorId: live.session.user_id, actor: live.session,
    });
