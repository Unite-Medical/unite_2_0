...
export function planOrganizationUpdate(organization, patch, { actorId, now = new Date() } = {}) {
  ...
  const migrated = organization.source === 'shopify_migration' || organization.shopify_customer_id;
  if (firstApproval && !migrated) {
    normalized = {
      ...normalized,
      tier: 'C',
      terms: 'ach',
  ...
  return {
    ...
    ensure_ach: firstApproval && !migrated,
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if ((/^net/.test(body.patch?.terms || '') || Number(body.patch?.credit_limit) > 0) && live.session.email?.toLowerCase() !== 'damon@unitemedical.net') return sendJson(res, 403, { error: 'damon_credit_approval_required' });
