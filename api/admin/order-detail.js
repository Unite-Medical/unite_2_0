...
export default async function handler(req,res){
 ...
 try{const live=await authorizeLiveRequest(req,sql,{roles:['admin']});if(!live.ok)return sendJson(res,403,{error:live.reason});const id=String(req.query.order_id||'');
  ...
  return sendJson(res,200,{ok:true,order:orders[0].data,items:rows.filter(r=>r.tbl==='order_items').map(r=>r.data),events:rows.map(r=>({id:r.data.id,type:r.tbl,status:r.data.status||r.data.kind||'',at:r.data.updated_at||r.data.created_at||r.data.placed_at||null,description:r.data.subject||r.data.name||r.data.reason||'',actor:r.data.actor_id||r.data.owner_email||null}))});
