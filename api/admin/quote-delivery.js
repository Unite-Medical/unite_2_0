...
export default async function handler(req,res){
 ...
 try{const sql=neon(process.env.DATABASE_URL),live=await authorizeLiveRequest(req,sql,{roles:['admin','sales','sales_manager']});if(!live.ok)return sendJson(res,403,{error:live.reason});
 ...
 const saved=await atomicTransition(sql,{checks:[{table:'quotes',id:before.id,before},{table:'organizations',id:org.id,before:org},{table:'addresses',id:address.id,before:address},...items.map(i=>({table:'quote_items',id:i.id,before:i}))],writes:[{table:'quotes',before,data:next},{table:'audit_log',data:audit}]});
 if(!saved.ok)return sendJson(res,409,{error:saved.reason});return sendJson(res,200,{ok:true,quote:projectCommercialRecord(next,live.session)});
