...
export function planPaidOrderRelease({ order, items = [], inventory = [], lots = [], ownerLots = [], reservations = [], now = new Date() } = {}) {
  ...
  if(['cancelled','canceled','refunded','delivered','closed'].includes(order.status))return {ok:false,reason:'order_closed'};
...
export function planOrderHandoff({
  ...
  if(['cancelled','canceled','refunded','delivered','closed'].includes(order.status))return {ok:false,reason:'order_closed'};
  const approvalGate=orderApprovalGate(order); if(!approvalGate.ok)return approvalGate;
  const paymentGate=paymentReleaseGate(order); if(!paymentGate.ok)return paymentGate;
  ...
  if (order.shipment_plan?.length > 1) return { ok: false, reason: 'split_shipment_fulfillment_review_required' };
...
export async function releasePaidOrder(sql, orderId, { actorId = 'payment_webhook', now = new Date() } = {}) {
  ...
  const paymentGate=paymentReleaseGate(orders[0]); if(!paymentGate.ok)return paymentGate;
  if(orders[0]?.shipment_plan?.length>1)return {ok:false,reason:'use_shipment_workspace'};
