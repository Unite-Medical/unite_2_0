...
export async function createOrderLabel(sql, orderId, { actorId = 'system', fetchImpl = fetch } = {}) {
  ...
  if (!provider.ok) {
    await upsert(sql, 'label_operations', { ...operation, status: ['shipstation_not_configured'].includes(provider.reason)||(provider.status>=400&&provider.status<500&&provider.status!==408&&provider.status!==429)?'failed':'provider_unknown', error: provider.reason, failed_at: new Date().toISOString() });
    return provider;
  }
