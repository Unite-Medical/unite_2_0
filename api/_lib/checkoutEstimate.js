import { ordinaryParcelCharge } from './launchPolicy.js';
...
export async function calculateEstimate({draft,products,organization,fetchImpl=fetch,env=process.env,now=new Date()}){
 ...
 for(const rate of rates){
  ...
  const charge=ordinaryParcelCharge(carrierCost,{arrangement:{markup_pct:organization.shipping_markup_pct??20,handling_flat:handling}});
  if(!charge.ok)return charge;
  const freight=charge.final_amount;
