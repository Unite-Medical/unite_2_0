import { planRefundApproval, refundApprovalGate } from '../_lib/refundApproval.js';
...
export function planRmaInspection({
  ...
  const feeRate = rma.reason === 'discretionary' && disposition === 'restock_sellable' ? 0.15 : 0;
...
export function planRmaRefund(rma, { evidence, actorId, actorRole, now = new Date() } = {}) {
  ...
  const approval = refundApprovalGate(rma); if (!approval.ok) return approval;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const plan = ['review_refund', 'approve_refund'].includes(action)
      ? planRefundApproval(rma, { action, actor: live.session, reason: body.reason })
      : planRmaStateAction(rma, { action, actorId: live.session.user_id });
    if (!plan.ok) return sendJson(res, 409, { error: plan.reason });
