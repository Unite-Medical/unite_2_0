...
| 2026-06-11 | **Durable persistence shipped (PRD-13 interim)** — `/api/db/sync` (Neon Postgres over HTTP, JSONB row-store mirroring the app's tables) + `src/lib/remoteDb.js`: boot hydration, debounced write-through of every local mutation, 20s incremental pull. Set `DATABASE_URL` + server-only `DB_SYNC_TOKEN`; signed privileged sessions and server jobs can use durable persistence without a browser credential. `npm run db:migrate` applies the 18 relational blueprints for the eventual dedicated-API cutover. |
...
- [ ] **Postgres (durable persistence)**: create a Neon project, set `DATABASE_URL` + server-only `DB_SYNC_TOKEN` (any long random string). Signed privileged sessions and automated server jobs can then use durable persistence without exposing a browser credential. Optionally run `npm run db:migrate` to lay down the relational schema for the future API tier.
