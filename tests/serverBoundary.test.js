...
test('live profile authorization revokes disabled, demoted, moved, and revision-stale sessions', () => {
  const session = { user_id: 'usr_admin', role: 'admin', org_id: 'org_unite', session_revision: 4, mfa_verified: true };
...
test('product price changes enforce named authority while preserving unchanged existing prices', () => {
  const jacobe={role:'sales',email:'jacobe@unitemedical.net'},damon={role:'admin',email:'damon@unitemedical.net'};
  assert.equal(validateAdminProductMutation({}, {sku:'A',price:100},'upsert',damon).reason,'product_cost_and_price_required');
  assert.equal(validateAdminProductMutation({sku:'A',cost:70,price:120},{price:100},'upsert',jacobe).reason,'damon_approval_required');
  assert.equal(validateAdminProductMutation({sku:'A',cost:70,price:120},{price:100},'upsert',damon).ok,true);
  assert.equal(validateAdminProductMutation({sku:'A',cost:70,price:80},{name:'New name'},'upsert',{role:'admin',email:'other@example.com'}).ok,true);
...
test('every signed-session route validates current live authority', async () => {
  ...
  for (const name of names) {
    ...
    assert.match(
      source,
      /authorizeLiveRequest|authorizeLiveProfile|loadCommerceContext|authorizeCommerceContext|finCustomerContext/,
