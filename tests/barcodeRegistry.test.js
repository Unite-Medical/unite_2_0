...
test('cleanup queue prioritizes launch variants with operational exposure',()=>{
 const products=[{handle:'launch',launch_decision:'Launch',variants:[{sku:'QA-STOCK'},{sku:'QA-EMPTY'}]},{handle:'later',launch_decision:'Hold',variants:[{sku:'QA-LATER'}]}];
 const queue=buildBarcodeCleanupQueue(products,[{SKU:'QA-STOCK','On hand (current)':2},{SKU:'QA-LATER','Incoming (not editable)':1}]);
 assert.deepEqual(queue.map(row=>row.priority),['launch_operational','launch_no_stock','not_launching']);
 assert.equal(queue[0].sku,'QA-STOCK');
});
