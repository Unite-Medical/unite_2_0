...
test('contract prices require an authoritative cost and enforce Jacobe’s 35 percent authority', () => {
  const approved = { ...organization, approval_status: 'approved' };
  const product = { sku: 'SKU-COSTED', landed_cost: 70, price: 120 };
  const tooLow = planContractPriceAction({
    organization: approved, product, actor:{role:'sales',email:'jacobe@unitemedical.net'}, action: 'set', unit_price: 99, min_qty: 1, actorId: 'usr_admin',
  });
  assert.equal(tooLow.reason, 'damon_approval_required');
  assert.equal(tooLow.minimum_price, 107.7);
  const valid = planContractPriceAction({
    organization: approved, product, actor:{role:'sales',email:'jacobe@unitemedical.net'}, action: 'set', unit_price: 107.70, min_qty: 1,
    actorId: 'usr_admin', now: new Date('2026-07-19T12:00:00.000Z'),
  });
